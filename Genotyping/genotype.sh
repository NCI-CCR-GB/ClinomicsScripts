...
DIR="/data/Clinomics/Analysis/Genotyping/"
#GenotypeFiles
#RATIO
rm -f $DIR/RATIO/*
da=`date +"%m-%d-%y"`
...
echo -e "echo -e \"Genotyping matrix and heatmap attached.\\\n\\\n\\\tSamples Processed \$samp\\\n\\\nRegards,\\\nClinOmics Team, NCI\" |mutt -s \"Genotyping Job Status\" -a MATRIX_${da}.txt -- patidarr@mail.nih.gov `whoami`@mail.nih.gov" >>Final

d=`swarm --partition quick -f $DIR/swarm --job-name GT`
final_run=`sbatch -o $DIR/gt.e --dependency=afterany:$d --partition=quick  $DIR/Final`
